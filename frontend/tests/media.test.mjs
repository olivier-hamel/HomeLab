...
import { sourceIntent, sortSources, bytes, sourceFingerprint, recommendedSources, loadDismissedSources, saveDismissedSources } from '../src/lib/media.ts';
...
test('rejecting a recommendation hides duplicate releases and promotes the next choice under every sort', () => {
  ...
  const advice = { ranking: ['a', 'duplicate', 'b', 'c'].map(id => ({ id })) };
