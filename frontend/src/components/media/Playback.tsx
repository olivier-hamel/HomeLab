...
function Player({ selection, initial, search, simple = false, onFailure, onNext, englishEnabled, onEnglishChange }: { selection: Selection; initial: TorrentStatus; search?: SearchIntent } & SimplePlayback) {
  ...
  const playerControls = <>
    ...
    <div className="tv-playback-controls flex flex-wrap items-center gap-3"><span role="status" className="mr-auto text-sm capitalize text-orange-400">{simple && state === "checking format" ? "Getting ready…" : state}</span>
