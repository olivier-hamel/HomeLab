  ...
  return <section className="media-profile-gate" aria-labelledby="profile-heading">
    <div className="media-profile-panel">
      ...
      <div className="media-profile-grid">
        {mediaProfiles.map((profile, index) => <button key={profile.id} type="button" style={{ "--tv-stagger": `${index * 65}ms` } as CSSProperties} className={`media-profile-card media-profile-${profile.color}`} aria-pressed={current === profile.id} onClick={() => choose(profile.id)}>
