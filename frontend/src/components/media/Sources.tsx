...
export default function Sources({ intent, choose, disabled }: { intent: SearchIntent; choose: (source: Source | string) => void; disabled: boolean }) {
  ...
  const search = (batch = 1) => { setResult(null); setPage(1); void task.run(s => mediaApi<SearchResults>("search", s, { query: q, batch, ...(useIds && q === intent.query ? { context: intent.context } : {}) }), setResult); };
  const sorted = sortSources(result?.results ?? [], sort);
  const pages = Math.max(1, Math.ceil(sorted.length / 25));
  return <div className="space-y-5">
    {intent.label && <div className="rounded border border-orange-500/30 bg-orange-500/5 p-4 text-sm"><p>Looking for <strong className="text-white">{intent.label}</strong></p><p className="mt-1 text-neutral-400">Review the query below. Each result still needs a title, year and episode check.</p></div>}
    ...
    <p className="text-xs leading-relaxed text-neutral-400">S = seeders. Leechers and peers are shown only as supplied by the indexer. Counts can be stale and do not predict playback speed.</p>
    {result && <>
      ...
      <div className="space-y-3">{sorted.slice((page - 1) * 25, page * 25).map(source => <article key={source.id} className="flex flex-col gap-4 rounded-lg border border-neutral-700 bg-neutral-900 p-4 lg:flex-row lg:items-center">
        ...
        <Button className="shrink-0 border border-orange-500/40 bg-orange-500/10 text-orange-400 hover:bg-orange-500/20" disabled={disabled} onClick={() => choose(source)}>Choose source</Button>
      </article>)}</div>
