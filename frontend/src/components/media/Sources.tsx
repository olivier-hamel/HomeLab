...
import { bytes, loadDismissedSources, mediaApi, recommendedSources, saveDismissedSources, sourceFingerprint, type SearchIntent, type SearchResults, type Source, type SourceAdvice } from "../../lib/media";
...
export default function Sources({ intent, choose, disabled, reject }: { intent: SearchIntent; choose: (source: Source | string, search?: SearchIntent) => void; disabled: boolean; reject: (source: Source) => void }) {
  ...
  const best = sorted[0] && assessments.get(sorted[0].id);
  const hiddenCount = (result?.results.length ?? 0) - sorted.length;
  const pages = Math.max(1, Math.ceil(sorted.length / 25));
  return <div className="space-y-5">
    {intent.label && <div className="rounded border border-orange-500/30 bg-orange-500/5 p-4 text-sm"><p>Looking for <strong className="text-white">{intent.label}</strong></p><p className="mt-1 text-neutral-400">Aiming for smooth web playback at 1080p, balancing format, seeders, size and listing quality.</p></div>}
    ...
    {result && <>
      ...
      {notice && <p role="status" className="text-xs text-neutral-400">{sorted.length ? notice : "All loaded sources are hidden; restore them or try another search."}</p>}
      ...
      <div className="space-y-3">{sorted.slice((page - 1) * 25, page * 25).map(source => {
        ...
        return <article key={source.id} aria-label={`${recommended ? "Recommended source: " : "Source: "}${source.title}`} className={`flex flex-col gap-4 rounded-lg border p-4 lg:flex-row lg:items-center ${recommended ? "border-orange-400/70 bg-orange-500/5 ring-1 ring-orange-500/20" : "border-neutral-700 bg-neutral-900"}`}>
        ...
        <div className="min-w-0 space-y-3 lg:w-72 lg:shrink-0">
          {assessment && <div className="space-y-1.5"><div className="flex flex-wrap items-center gap-2 text-xs">{recommended && <span className="flex items-center gap-1 font-semibold text-orange-300"><Sparkles className="h-3.5 w-3.5" />{assessment.verdict === "sketchy" ? "Least concerning" : "Best available"}</span>}<span className={`rounded px-2 py-0.5 font-medium ${assessment.verdict === "good" ? "bg-emerald-500/15 text-emerald-300" : assessment.verdict === "sketchy" ? "bg-rose-500/15 text-rose-300" : "bg-amber-500/15 text-amber-300"}`}>{assessment.verdict === "good" ? "Good" : assessment.verdict === "sketchy" ? "Sketchy" : "Unsure"}</span><span className="text-neutral-500">{assessment.method === "gemini" ? "AI" : "Basic checks"}</span></div><p className="break-words text-xs leading-relaxed text-neutral-300">{assessment.reason}</p></div>}
